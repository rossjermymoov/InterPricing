...
class CrownClient {
  ...
  getCredentials() {
    ...
    return {
      baseUrl: (settings.apiBaseUrl || process.env.CROWN_API_BASE_URL || 'https://api.crownsds.com').replace(/\/+$/, ''),
      clientId: settings.clientId || process.env.CROWN_CLIENT_ID || '',
      clientSecret: settings.clientSecret || process.env.CROWN_CLIENT_SECRET || '',
      defaultCustomerID: settings.defaultCustomerID || process.env.CROWN_CUSTOMER_ID || 'DEMO01',
      useMock: settings.useMockApi === true || settings.useMockApi === 'true' || !settings.clientId
    };
  }
