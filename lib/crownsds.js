...
class CrownClient {
  ...
  async requestQuote(payload) {
    ...
    try {
      ...
      console.log(`[CrownClient] Sending live quote request to ${creds.baseUrl}/v1/api/crownQuote for ${payload.vanSize} with ${payload.stations?.length} stops...`);
      ...
      const data = await response.json();
      console.log('[CrownClient] Live Crown Quote response received:', JSON.stringify(data));
      // Normalize price from any casing returned by the API
      const rawPrice = data.price ?? data.Price ?? data.total ?? data.cost ?? data.amount ?? data.totalPrice;
      return {
        ...data,
        price: rawPrice !== undefined ? String(rawPrice) : data.price,
        reference: data.reference || data.Reference || data.quoteReference || data.quote_reference || data.quoteId || 'CRW-LIVE'
      };
    } catch (err) {
      console.warn('[CrownClient] Live Quote API failed, generating fallback quote:', err.message);
      return this.generateMockQuote(payload);
    }
  }
