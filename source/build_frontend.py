...
<div class="calcrow">
  ...
  <div class="f small"><label>Markup %</label><input id="markup" type="number" min="0" step="1" value="0"/></div>
</div>
...
<div class="panel" id="reportPanel">
  ...
  <p class="chartnote" style="margin:0 0 10px">Select destinations below. Uses the weight, dimensions, value, <b>markup</b> and options set above. Run to preview, open a printable report, or export a full customer rate card to Excel. With destinations selected the export is limited to them; with none selected it exports every country.</p>
...
function build(rawBase,svc){
  ...
  const mk=markupPct(), markupAmt=sellShip*mk/100;
...
function bootCalc(){
  ...
  if(!csel.options.length){
    ...
    ['country','wt','L','W','H','goodsValue','markup','metric'].forEach(id=>$(id).addEventListener('input',calc));
...
function cardSell(svc,rawBase){
  ...
  const f=fuelOf(svc),mk=markupPct();
...
function exportRateCard(){
  ...
  const cover=[
    ...
    [],
    ['All rate-sheet prices are customer sell prices in GBP, including fuel and the markup above, excluding VAT.'],
    ...
    [],
    ['Fuel by service (sell %)'],
  ];
  SERVICES.forEach(svc=>cover.push([svc.name,fuelOf(svc).sell+'%']));
  add(cover,'Rate Card');
  ...
  XLSX.writeFile(wb,'Rate card '+now.replace(/ /g,'-')+'.xlsx');
}
