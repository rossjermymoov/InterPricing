...
function renderQuoteLog(rows){
  ...
  rows.forEach(x=>{
    ...
    dr.innerHTML='<td></td><td colspan="6"><table><thead><tr><th>Service</th><th>Transit</th><th>Cost</th><th>Quoted price</th></tr></thead><tbody>'+svcRows+'</tbody></table>'
      +(x.sender_company?'<div class="qmuted" style="margin-top:6px">Sender: '+qesc(x.sender_company)+'</div>':'')+'</td>';
    tb.appendChild(dr);
  });
...
$('saveBtn').onclick=async()=>{$('saveMsg').className='ok';$('saveMsg').textContent='';
  ...
  const r=await jput('/api/settings',{fuelByService:fbs,accessorials:acc,euCustomsDuty,importMarkupPct});const d=await r.json();
