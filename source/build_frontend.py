...
function accAmount(a,svc){const amt=(a.applyTo==='all')?accNet(a):(svc.carrier==='ups'?accNet(a):accDpd(a));return Math.round((amt||0)*100)/100;}
...
function buildAccTable(){
  ...
  const trg=a=>a.cond==='auto'?'auto (size/wt)':(a.cond==='always'?'always':(a.cond==='countryIn'?(a.countries||[]).join(','):'toggle'));
  accList().forEach(a=>{const tr=document.createElement('tr');
    tr.innerHTML=`<td>${a.name}</td><td><span class="tagpill${a.cond!=='toggle'?' auto':''}">${trg(a)}</span></td>
